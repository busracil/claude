...
# ─── Debug: Streamlit Cloud'da hangi yolda çalıştığını gösterir ──
with st.sidebar.expander("🔧 Yol Bilgisi (debug)", expanded=False):
    st.code(f"OUTPUT_DIR:\n{C.OUTPUT_DIR}\n\nDASHBOARD:\n{C.DASHBOARD_PATH}")
    if os.path.exists(C.OUTPUT_DIR):
        files = os.listdir(C.OUTPUT_DIR)
        st.success(f"outputs/ bulundu — {len(files)} dosya")
        st.write(files)
    else:
        st.error("outputs/ klasörü BULUNAMADI")
